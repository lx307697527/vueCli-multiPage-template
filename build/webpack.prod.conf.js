...
var webpackConfig = merge(baseWebpackConfig, {
  ...
  plugins: [
    ...
    new webpack.DefinePlugin({
      'process.env': env,
      'DISTTIME': JSON.stringify(utils.formatTimestamp(new Date().getTime())),
    ...
    // removeComments: true,     collapseWhitespace: true, removeAttributeQuotes:
    // true     // more options:     //
    // https://github.com/kangax/html-minifier#options-quick-reference   },   //
    // necessary to consistently work with multiple chunks via CommonsChunkPlugin
    // chunksSortMode: 'dependency' }), split vendor js into its own file
    new webpack
    .optimize
    .CommonsChunkPlugin({
      name: 'vendor',
    ...
    new FaviconsWebpackPlugin({
      ...
      logo: resolve('src/assets/images/merchant/favicon-logo.png'),
      // The prefix for all image files (might be a folder or a name)
      prefix: 'static/icons-[hash]/',
      // Emit all stats of the generated icons
      emitStats: false,
      // The name of the json containing all favicon information
      statsFilename: 'static/iconstats-[hash].json',
...
webpackConfig.entry = getEntry('./src/page/**/*.js');
...
for (var pathname in pages) {
  ...
  var conf = {
    ...
    filename: pathname + '.vm',
    template: pages[pathname], // 模板路径
    chunks: [
      pathname, 'vendor', 'manifest'
    ], // 每个html引用的js模块
    inject: true,
    title: config.build.project,
    desc: config.build.desc,
    keywords: config.build.keywords,
    staticPlaceHolder: config.build.staticPlaceHolder,
  };
