...
const api = {
  target: 'http://func.yanxiajinrong.com/',
  changeOrigin: true,
  cookieDomainRewrite: {
    '*': ''
  }
...
module.exports = {
  build: { // production 环境
    project: '',
    desc: '',
    staticPlaceHolder: 'window.STATIC_DOMAIN = "${staticServer}/${partnerName}/"',
    keywords: '',
    env: require('./prod.env'), // 使用 config/prod.env.js 中定义的编译环境
    index: path.resolve(__dirname, '../dist/index.html'), // 编译输入的 index.html 文件
    assetsRoot: path.resolve(__dirname, '../dist'), // 编译输出的静态资源路径
    pageDirectory: 'page', // vm模板输出目录
    assetsSubDirectory: 'static', // 编译输出的二级目录
    assetsPublicPath: '_STATIC_DOMAIN_PLACEHOLDER_', // 编译发布的根目录占位符，打包结束之后替换为java变量,见/build/replace.js
